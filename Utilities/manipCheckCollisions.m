function [isInCollision, selfCollisionPairIdx, worldCollisionPairIdx] = manipCheckCollisions(tree, bodyCollisionArray, worldCollisionArray, config, isExhaustiveChecking)
% 机械臂自身关节碰撞

%manipCheckSelfCollisions Check for collisions between robot bodies
...
%   Copyright 2019 The MathWorks, Inc.
    ...
    % Basic validation
    narginchk(5,5)
    validateattributes(tree, {'robotics.RigidBodyTree'}, {'nonempty'}, 'manipSelfCollisions', 'tree');
    validateattributes(bodyCollisionArray, {'cell'}, {'nonempty','nrows',tree.NumBodies, 'ncols', 2}, 'manipSelfCollisions', 'tree');
    validateattributes(config, {'double'}, {'nonempty','vector','nrows',length(tree.homeConfiguration)}, 'manipSelfCollisions', 'config');
    validateattributes(isExhaustiveChecking, {'logical'}, {'nonempty','scalar'}, 'manipSelfCollisions', 'isExhaustiveChecking');
    ...
    % the base frame
    transformTree = cell(numel(robotBodies)-1,1); 

    % For the base, this is the identity
    transformTree{1} = eye(4);
    for i = 1:numel(robotBodies)-1
        transformTree{i} = getTransform(tree, config, robotBodies{i}.Name);
    end
    
    % 遍历所有机械臂关节，除机械臂末端
    for j = 1:numel(robotBodies)-1
        % Since collisionPairIdx order doesn't matter, only check every pair of bodies once
        for k = j:numel(robotBodies)-1
